...
## その他のボタンなど
- 「上書き保存」		
    - 譜面をファイルに上書き保存する。ファイルは最初から読み込まれている場合はそのファイルに上書き保存、新規作成の場合はmp3ファイルと同じディレクトリに保存される。idがファイル名になる。
- 「外部編集」		
...
このツールでできないorやりにくい編集をしたい場合は「外部編集」でテキストエディタなどで譜面を開いて編集し、上書き保存した上で、本ツールで再読み込みすると良いです。  
